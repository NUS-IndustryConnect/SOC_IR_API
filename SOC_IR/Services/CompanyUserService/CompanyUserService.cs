...
namespace SOC_IR.Services.CompanyUserService
{
    public class CompanyUserService : ICompanyUserService
    {
        ...
        public async Task<ServiceResponse<List<CompanyUserDto>>> UpdateCompanyUser(UpdateCompanyUserDto updateDto)
        {
            ...
            _context.CompanyUsers.Update(user);
            await _context.SaveChangesAsync();
            List<CompanyUserDto> newList = await _context.CompanyUsers.Where(a => a.companyId == updateDto.companyUserId).Select(a => new CompanyUserDto(a.companyUserId, a.companyId, a.companyName, a.email, a.lastLoggedIn, a.isActive)).ToListAsync();
            response.Data = newList;
            return response;
        }
    }
